...
public class Bottle {
    ...
    public void drinkAmount(int amount) {
        if(!open) {
            System.out.println("Bottle is closed, cannot drink form it");
            return;
        }
        if(availableLiquid >= amount) {
            System.out.println(String.format("Am baut din sticla cantitatea %d si a ramas %d", amount, (availableLiquid-amount)));
